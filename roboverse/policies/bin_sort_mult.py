...
class BinSortNeutralMult:
    ...
    def __init__(self, env, pick_height_thresh=-0.31, xyz_action_scale=7.0,
                 pick_point_noise=0.00, drop_point_noise=0.00, neutral_tolerance=0.02, # forward should be 0.05 backwards 0.02,
    ...
    def reset(self):
        ...
        # else:
        self.object_to_target = self.env.object_names[0]

        self.get_pickpoint()
        self.drop_point = self.env.container_position if bin_sort_hash(self.object_to_target) % 2 == 0 else self.env.container2_position
        self.other = self.env.container2_position if bin_sort_hash(self.object_to_target) % 2 == 0 else self.env.container_position
        ...
        if self.env.bin_obj or self.env.get_task() == "backward":
            ...
            # print(self.drop_point)
            self.drop_point[2] = -0.17
        self.drop_point[2] = -0.2 # for forward
    ...
    def get_pickpoint(self):
        ...
        self.pick_point[2] = -0.32#-0.32 for forward 0.3 for backward
    ...
    def get_action(self):
        ...
        if self.place_attempted:
            ...
            if xy_diff > 0.03: #0.015 for backward 0.03 forward
                action_xyz[2] = 0.0
            action_angles = [0., 0., 0.]
            action_gripper = [0.0]
