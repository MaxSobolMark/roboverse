...
class Widow250PickPlaceEnv(Widow250Env):
    ...
    def __init__(self,
                 ...
                 random_intialization_bins=5,
                 ee_pos_margin=0.05, # don't allow the gripper to be too close to the edge of the workspace
                 rng_seed=0,
                 **kwargs
                 ):
        ...
        min_pos, max_pos = np.array(self.ee_pos_low), np.array(self.ee_pos_high)
        
        min_pos = min_pos + ee_pos_margin
        max_pos = max_pos - ee_pos_margin
        assert np.all(min_pos < max_pos), f"min_pos {min_pos} must be less than max_pos {max_pos}, margin is {ee_pos_margin}"
        
        # ensure that the starting position is above the table
        min_pos[-1] = max(max(min_pos[-1], container_config['container_position_low'][-1]), container_config['place_success_height_threshold'])
        max_pos[-1] = max(max(max_pos[-1], container_config['container_position_high'][-1]), container_config['place_success_height_threshold'])

        diff = np.array(max_pos) - np.array(min_pos)
        bin_size = diff / random_intialization_bins

        assert len(min_pos) == len(max_pos) == 3
        locs = np.mgrid[min_pos[0]:max_pos[0]:bin_size[0],
                        min_pos[1]:max_pos[1]:bin_size[1],
                        min_pos[2]:max_pos[2]:bin_size[2]].reshape(3,-1).T
        locs = locs + bin_size / 2 # center the bins

        prng = np.random.RandomState(rng_seed)
        prng.shuffle(locs)
        self.random_initialization_locs = locs
        self.rng_seed = rng_seed

    def reset(self):
        obs = super().reset()
        if hasattr(self, 'random_initialization') and self.random_initialization > 0:
            desired_ee_pos = self.random_initialization_locs[self.rng_seed % len(self.random_initialization_locs)]
            self.rng_seed += 1

            print('current_ee_pos', obs['state'][:3])
            print('desired_ee_pos', desired_ee_pos)

            for _ in range(max(self.random_initialization, 0)):
                curr_ee_pos = obs['state'][:3]
                action_scale=1
                ee_dif = desired_ee_pos - curr_ee_pos
                print('ee_dif', ee_dif)
                quat_dif = np.zeros((3,))
                gripper = np.array([1.0])
                action = np.concatenate([ee_dif, quat_dif, gripper]) * action_scale
                obs, *_ = self.step(action)
            print('random_ee_pos', obs['state'][:3])

            self.is_gripper_open = bool((gripper > 0.5).item())
